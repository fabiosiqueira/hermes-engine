...
def is_curation_eligible(skill_name: str, skill_path: Optional[Path] = None) -> bool:
    ...
    if local_dir is not None and _frontmatter_locks_curation(local_dir):
        return False
...
def _frontmatter_locks_curation(skill_path: Path) -> bool:
    """True when the skill's own SKILL.md opts out of curation via ``metadata.hermes.pinned`` or
    ``metadata.hermes.locked``. Fail-open like ``_locked_guard`` in skill_manager_tool.py: a missing or
